...
CALL add_elk_dialogue_text ("ERONA_INTRO_1", "I hope you enjoyed the last sleep of your jaunt in Sunstrider, as the isle won't be able to host you anymore. The incident involving Felendren has gotten worse than initially estimated and has festered beyond any of our possessed remedies.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_1A", "It is always sad when pursuit of knowledge is cut short, professor.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_1B", "Dammed he be, this was supposed to be a leisurely excursion in which I wore only a chrisom.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_1C", "I wish to fawn over the details of how the Sin'dorei perservere through uncommon cosmic circumstances. Could you tell me about it?");
CALL add_elk_dialogue_text ("ERONA_TALK_2", "Malevolent destruction protrudes from Falthrien Academy as a worm from a corpse of an animal slaughtered by a predator it could not understand. It is of origin too alien for our experts to ascertain correctly, tainting the Arcane and Fel substances of the isle with deathly fashion.$b$bThese rumbling qualities correlate with death and mingle as symbiosis of blossoms with bees, yet bearing synthesis that there is no well fashioned theorems on.$b$bThe destruction of the Sunwell by the Scourge sent adrift numerous disturbances that push us towards annihilation teetering on hideous oblivion, and this here unnatural event is no less cruel nor merciless.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_2A", "Is there any way I can be of assistance? It is my ethical duty as a Sin'dorei to help anything that endangers our race.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_2B", "Well, I could always be the first to pin the name for it down. Where should I start?");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_2C", "Why are there so few guards or Blood Knights dealing with this?");
CALL add_elk_dialogue_text ("ERONA_TALK_2C", "Three Blood Kngihts had already gone up the Academy to help some of the students, MY students, all were killed by the apparitions, and the Wretched in west Silvermoon have become more rabid, cutting off reinforcements. Until we have a better grasp on the situation, it is unsafe to approach Felendren's madness so lightly.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_2CA", "My condolences about your students, are you well?");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_2CB", "You failed to defend your students? You are an embarassment to the Magisters.");
CALL add_elk_dialogue_text ("ERONA_TALK_2CAA", "I'm a bit shaken, but I can spare no tears for it is not something I'm not used to seeing.");
CALL add_elk_dialogue_text ("ERONA_TALK_2CBA", "Do not whip with your tongue so lightly, if you weren't speaking the truth your throat would be gagged by it.");
CALL add_elk_dialogue_text ("ERONA_TALK_3", "Ask around, Helion and Lanthan are the most engaged with the issue but everyone could use some work done for them. How much experience or education as a warrior or magician do you have? Don't answer, I can see for myself.$b$bIf you wish to aid us with this hope-dispersing disaster and pull together what high hopes we had for Sunstrider Isle then you should learn how to deal with cunning combat.$b$b");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_3A", "<Continue>");
CALL add_elk_dialogue_text ("ERONA_TALK_4", "To aid us purging this influence I recommend you arm and gear yourself in the Thelyrn Sister's store, and see who can help or be helped over the issue in question.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_4A", "I'll see what I can do.");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_4B", "This disaster killed friends of mine, I shall not rest until Quel'Thalas is cleansed from it.");
CALL add_elk_dialogue_text ("ERONA_INTRO_5", "$n, anything I can help you with? Information, or perhaps any favours?");
CALL add_elk_dialogue_text ("ERONA_DIALOGUE_5", "What should I do if I want to help around?");
CALL add_elk_dialogue_text ("ERONA_INTRO_6", "Greetings, stranger. Welcome to Sunstrider Isle.");
...
CALL add_elk_dialogue_text ("ESTHEN_INTRO_1", "Greetings, $n");
...
CALL add_elk_dialogue_text ("FELENDREN_DIALOGUE_1D", "Why were you here anyway, bastard?");
CALL add_elk_dialogue_text ("FELENDREN_DIALOGUE_1E", "<Attack> I finished up my preparation, time to die...");
