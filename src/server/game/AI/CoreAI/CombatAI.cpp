...
void ArcherAI::AttackStart(Unit* who)
{
    ...
    if (me->IsWithinMeleeRange(who, m_minRange))
    {
...
void ArcherAI::UpdateAI(uint32 /*diff*/)
{
    ...
    if (!me->IsWithinMeleeRange(me->GetVictim(), m_minRange))
...
bool TurretAI::CanAIAttack(Unit const* /*who*/) const
{
    ...
    if (!me->IsWithinMeleeRange(me->GetVictim(), me->m_CombatDistance)
            || (m_minRange && me->IsWithinMeleeRange(me->GetVictim(), m_minRange)))
        return false;
    return true;
}
