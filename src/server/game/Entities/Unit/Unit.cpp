...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool /*allowGM*/, Spell* damageSpell /*= nullptr*/)
{
...
SpellCastResult Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster, std::map<MapDummy, std::optional<std::any>> map)
{
    if (!spellInfo)
    {
    ...
    auto ref = spell->GetTriggerDummy();
    ref = map;
...
SpellCastResult Unit::CastCustomSpell(Unit* target, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster, std::map<MapDummy, std::optional<std::any>> map)
{
    ...
    return CastCustomSpell(spellId, values, target, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura, originalCaster, map);
}

SpellCastResult Unit::CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* target, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster, std::map<MapDummy, std::optional<std::any>> map)
{
    CustomSpellValues values;
...
SpellCastResult Unit::CastCustomSpell(uint32 spellId, SpellValueMod mod, int32 value, Unit* target, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster, std::map<MapDummy, std::optional<std::any>> map)
{
    CustomSpellValues values;
    values.AddSpellMod(mod, value);
    return CastCustomSpell(spellId, values, target, triggerFlags, castItem, triggeredByAura, originalCaster, map);
}

SpellCastResult Unit::CastCustomSpell(uint32 spellId, CustomSpellValues const& value, Unit* victim, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster, std::map<MapDummy, std::optional<std::any>> map)
{
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
    if (!spellInfo)
    {
    ...
    return CastSpell(targets, spellInfo, &value, triggerFlags, castItem, triggeredByAura, originalCaster, map);
}
...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss, Spell* spell /*= nullptr*/)
{
    if (damageInfo == 0)
        return;
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, Spell* spell, bool CanReflect)
{
...
Spell* Unit::FindCurrentSpellBySpellId(uint32 spell_id)
{
    for (uint32 i = 0; i < CURRENT_MAX_SPELL; i++)
...
int32 Unit::GetCurrentSpellCastTime(uint32 spell_id)
{
    if (Spell* spell = FindCurrentSpellBySpellId(spell_id))
...
void Unit::ProcDamageAndSpell(Unit* actor, Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procExtra, uint32 amount, WeaponAttackType attType, SpellInfo const* procSpellInfo, SpellInfo const* procAura, int8 procAuraEffectIndex, Spell* procSpell, DamageInfo* damageInfo, HealInfo* healInfo, uint32 procPhase)
{
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, ProcEventInfo const& eventInfo)
{
    ...
    Spell* spellProc = eventInfo.GetProcSpell();
            ...
            case SPELLFAMILY_SHAMAN:
                {
                    ...
                    if (auraSpellInfo->SpellFamilyFlags[0] & 0x400 && auraSpellInfo->HasAttribute(SPELL_ATTR1_NO_THREAT))
                    {
                        ...
                        if (Spell* spell = eventInfo.GetProcSpell())
                        {
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint8 effIndex, float TotalMod, uint32 stack)
{
    ...
    DoneAdvertisedBenefit += SpellBasePowerBonusDone(spellProto->GetSchoolMask());
    ...
    if (victim)
    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            if (!spellProto->IsPositive())
            {
                ...
                if (victim->HasAuraType(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE))
                {
                    crit_chance += victim->GetMaxNegativeAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE, schoolMask);
                    crit_chance += victim->GetMaxPositiveAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_CHANCE, schoolMask);
                }
            }
        ...
        case SPELL_DAMAGE_CLASS_RANGED:
        {
            // flat aura mods
            if (attackType == RANGED_ATTACK)
                crit_chance += victim->GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_RANGED_CRIT_CHANCE);
            else
                crit_chance += victim->GetTotalAuraModifier(SPELL_AURA_MOD_ATTACKER_MELEE_CRIT_CHANCE);


            crit_chance += (int32(GetMaxSkillValueForLevel(this)) - int32(victim->GetDefenseSkillValue(this))) * 0.04f;
...
bool Unit::IsImmunedToDamage(Spell* spell) const
{
    SpellInfo const* spellInfo = spell->GetSpellInfo();
    if (!spellInfo)
    {
...
bool Unit::IsImmunedToSchool(Spell* spell) const
{
    SpellInfo const* spellInfo = spell->GetSpellInfo();
    if (spellInfo->HasAttribute(SPELL_ATTR0_NO_IMMUNITIES) && !HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION))
    {
...
bool Unit::IsImmunedToSpell(SpellInfo const* spellInfo, Spell* spell)
{
    if (!spellInfo)
        return false;
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index, int32 const* basePoints)
{
    ...
    switch (power)
    {
        ...
        case POWER_ENERGY:
            if (this->GetTypeId() == TYPEID_PLAYER)
                return this->GetStat(STAT_STAMINA) * 10;
            else
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpellInfo, uint32 damage, SpellInfo const* procAura, int8 procAuraEffectIndex, Spell* procSpell, DamageInfo* damageInfo, HealInfo* healInfo, uint32 procPhase)
{
...
void Unit::Kill(Unit* killer, Unit* victim, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const* spellProto, Spell* spell /*= nullptr*/)
{
...
float Unit::SpellTakenCritChance(Unit const* caster, SpellInfo const* spellProto, SpellSchoolMask schoolMask, float doneChance, WeaponAttackType attackType, bool skipEffectCheck)
{
    // not critting spell
    if (spellProto->HasAttribute(SPELL_ATTR2_CANT_CRIT))
        return 0.0f;
